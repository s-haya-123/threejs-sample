...
export async function predictImages(model:tf.LayersModel, imgs: HTMLImageElement[]): Promise<Picture[] | undefined> {
    ...
    return predictImagesFromTensor(model,bathcedImgs);
}
...
export async function predictImagesFromTensor(model:tf.LayersModel, bathcedImgs: tf.Tensor<tf.Rank>[]): Promise<Picture[] | undefined> {
...
export async function predictImageFromTensor(model:tf.LayersModel, concatBatchedImgs: tf.Tensor<tf.Rank>): Promise<Picture[] | undefined> {
    console.log(await model.predict(concatBatchedImgs));
    const predicts = await (model.predict(concatBatchedImgs) as tf.Tensor<tf.Rank>).array();
